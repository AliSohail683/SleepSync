...
class StorageService {
  ...
  async setupDB(): Promise<void> {
    try {
      ...
      await this.db.execAsync(`
        ...
        CREATE INDEX IF NOT EXISTS idx_sleep_session_user ON sleep_session(user_id);
        CREATE INDEX IF NOT EXISTS idx_sleep_session_date ON sleep_session(start_at);
        CREATE INDEX IF NOT EXISTS idx_sleep_debt_date ON sleep_debt_record(date);
      `);
  ...
  async deleteSoundProfile(profileId: UUID): Promise<void> {
    const db = this.getDB();
    await db.runAsync('DELETE FROM sound_profile WHERE id = ?', [profileId]);
  }
  ...
  async deleteAlarmConfig(configId: UUID): Promise<void> {
    const db = this.getDB();
    await db.runAsync('DELETE FROM alarm_config WHERE id = ?', [configId]);
  }
  ...
  async clearAllData(): Promise<void> {
    const db = this.getDB();
    await db.execAsync(`
      DELETE FROM sleep_session;
      DELETE FROM sound_profile;
      DELETE FROM alarm_config;
      DELETE FROM sleep_debt_record;
